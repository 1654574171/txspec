...
uint32_t ComplianceEngine::LoadEnclaves()
{
    uint32_t status;
    ...
    {
        std::cout << "ComplianceEngine: ENCLAVE_FILENAME is NULL" << std::endl;
...
void ComplianceEngine::ReportCreateSessionError (sgx_enclave_id_t source_enclave_id,
...
uint32_t ComplianceEngine::KeyManagementCreateSession (sgx_enclave_id_t dest_enclave_id) {
    uint32_t status = 0;
...
uint32_t ComplianceEngine::BindingCreateSession (sgx_enclave_id_t dest_enclave_id) {
...
uint32_t ComplianceEngine::RuleCreateSession (sgx_enclave_id_t dest_enclave_id) {
...
uint32_t ComplianceEngine::StartLocalAttesttion()
{
...
uint32_t ComplianceEngine::LaunchGRPCServer()
{
...
uint32_t ComplianceEngine::LaunchHttpClient() {
...
uint32_t ComplianceEngine::SafeLogout()
{
    // TODO how to logout safely?
    uint32_t status;

    status = sgx_destroy_enclave(g_key_management_enclave_id);
    if (status != SGX_SUCCESS)
    {
        SPDLOG_INFO("Key Management Enclave destroy failed: Error code is {}", status);

        // TODO should we exit early?
        return SGX_ERROR_UNEXPECTED;
    }

    status = sgx_destroy_enclave(g_rule_enclave_id);
    if (status != SGX_SUCCESS)
    {
        SPDLOG_INFO("Rule Enclave destroy failed: Error code is {}", status);

        // TODO should we exit early?
        return SGX_ERROR_UNEXPECTED;
    }

    status = sgx_destroy_enclave(g_binding_enclave_id);
    if (status != SGX_SUCCESS)
    {
        SPDLOG_INFO("Binding Enclave destroy failed: Error code is {}", status);
